...
- **Runs existing binaries unmodified.** No recompiling tools to a WASM target —
  full poppler (C++) runs with zero porting.
- Cold boot ~130 ms; common tools (shell, sqlite, pdftotext) run 3–11× faster
...
```jsonc
{
  "buildId": "…",            // snapshot etag; a rebuilt runtime busts stale snapshots
  "home": "/workspace",
  "applets": ["sh","ls", …], // busybox multi-call symlinks
  "bundles": {
    "base": { "tier": "eager", "files": [ {"url":"bin/busybox","dest":"/bin/busybox","mode":"0755","applets":true} ] },
    "oss":  { "tier": "lazy", "triggers": ["pdftotext","sqlite3", …],
              "files": [ {"url":"rootfs/bin/sqlite3","dest":"/bin/sqlite3","mode":"0755"}, … ] }
  }
}
```

- **eager** bundles stage at boot; **lazy** bundles stage on the first command
  matching a `triggers` token (or an explicit `vm.loadBundle`).
...
Guest HTTP clients (musl `ureq`, busybox `wget`) do the normal
`getaddrinfo → socket → connect → write → read`. The runtime implements virtual
sockets in blink: `connect()` to a seeded route IP hands the request to JS, which
routes it to your `registerVmEndpoint` handler (or a direct authed `fetch`, per
your `vmRoutes`), and streams the HTTP response back. Unknown hosts get a 403.

## Tests

```sh
bun test/contract.spec.mjs    # window.vm contract + loadBundle + writeFile-install
bun test/dist-smoke.spec.mjs  # packaging: cold boot, lazy bundle, buildId etag, snapshot
...
- **Raster is slow.** `pdftoppm` (scanned-PDF rasterization) is ~27 s/page in the
  interpreter; text extraction (`pdftotext`) is fast.
