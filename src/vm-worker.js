...
self.Module = {
  ...
  preRun: [function () {
    ...
    // the real one (/tmp/session_<session>). A HOME-valued fallback here would
...
// session: WORKDIR=/tmp/session_<sid> and SESSION_ID are exported for real (and
...
async function runShellCapture(cmd, capFile, pidFile, session) {
  ...
  if (session) {
    ...
    var wd = "/tmp/session_" + sid, sess = "/tmp/.sess_" + sid;
...
function makeSnapshot() {
  var files = {};
  walk(HOME, files);
  // /tmp persists too: session workdirs (/tmp/session_*), session state
