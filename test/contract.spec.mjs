...
try {
  ...
  check("session: WORKDIR + SESSION_ID exported", wdOut.output === "W=/tmp/session_sessA S=sessA", JSON.stringify(wdOut.output));
  const wdDir = await page.evaluate(() => window.vm.run("[ -d /tmp/session_sessA ] && echo exists", undefined, "sessA"));
  ...
  check("session: default cwd is the workdir", cwd0.output === "/tmp/session_sessA", JSON.stringify(cwd0.output));
  ...
  const tmpAfterReboot = await page.evaluate(() => window.vm.readFile("/tmp/session_sessA/keep.txt").catch(() => "<missing>"));
