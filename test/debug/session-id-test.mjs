...
const INIT_CMD = "[ -f /tmp/.mvl_boot ] || { uuidgen > /home/user/.session_id; : > /tmp/.mvl_boot; }";
...
async function bootAndInit() {
  ...
  await run(INIT_CMD);
  return (await run("cat /home/user/.session_id")).trim();
}
...
await run2(INIT_CMD);
const id2 = (await run2("cat /home/user/.session_id")).trim();
