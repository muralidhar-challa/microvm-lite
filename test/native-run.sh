...
#
#   test/native-run.sh mytool ping
#   test/native-run.sh pdftotext -f 1 -l 10 test/permit.pdf /tmp/out.txt
#
# Notes:
...
[ -x "$BLINK" ] || { echo "native blink not built: (cd ./blink-native && ./configure && gmake -j8 o//blink/blink)" >&2; exit 1; }
