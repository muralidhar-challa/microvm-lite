...
const BENCHES = [
  ...
  ["mytool ping", ["mytool", "ping"], 2000, "pong"],
  ["mytool excel_create", ["mytool", "excel_create", "/workspace/bench.xlsx", "--sheets=[\"S1\"]"], 10000, null],
  ["mytool excel_set_batch 50 cells", ["mytool", "excel_set_batch", "/workspace/bench.xlsx::S1",
      "--ops=" + JSON.stringify(Array.from({ length: 50 }, (_, i) => ({ col: (i % 5) + 1, row: ((i / 5) | 0) + 1, value: "v" + i }))),
    ], 10000, null],
  ["mytool excel_get col", ["mytool", "excel_get", "/workspace/bench.xlsx::S1/col/1"], 10000, "v0"],
